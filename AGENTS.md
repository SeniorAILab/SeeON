...
- Nursing-home RTSP 검증은 녹화 영상을 external `SeniorAILab/rtsp-generator` 또는 실제 카메라 endpoint로 반복 송출하고 `ml-worker -> backend /ingest/* -> DB side effect`를 확인한다. canned detector, fake backend, in-memory DB, stub ingest는 낙상 탐지 E2E 증거가 아니다.
