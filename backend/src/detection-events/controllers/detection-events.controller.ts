...
export class DetectionEventsController {
  ...
  @Patch(':id')
  update(@Req() req: RequestWithAuth): never {
    return notImplemented(req);
  }
}

function notImplemented(req: RequestWithAuth): never {
  requireFacilityId(req);
  throw new NotImplementedException({
    error: 'not_implemented',
    message: 'detection-events is not implemented yet',
  });
}
