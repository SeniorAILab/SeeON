...
@Controller()
export class LegacyAlertEventsController {
  constructor(
    private readonly configService: ConfigService,
    private readonly alertEventsService: AlertEventsService,
  ) {}

  // Temporary pilot/backcompat alias. Prefer POST /api.alerts/events.
  @Post('events')
  async createEvent(
    @Headers(ALERT_EVENTS_API_KEY_HEADER) apiKey: string | undefined,
    @Body() body: unknown,
...
function assertAuthorizedAlertEventIngress(
...
function parseAlertEventIngress(body: unknown): AlertEventIngressDto {
...
class AlertEventPayloadError extends UnprocessableEntityException {
