...
IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-ghcr.io/goberomsu/eldercare-fall-ai}"
IMAGE_TAG="${IMAGE_TAG:-latest}"
PULL_POLICY="${PULL_POLICY:-always}"

if [ "$DEPLOY_MODE" = "image" ]; then
  COMPOSE_FILES="-f compose.yaml -f compose.prod.yaml -f compose.registry.yaml"
  BACKEND_IMAGE="${BACKEND_IMAGE:-$IMAGE_REPOSITORY-backend:$IMAGE_TAG}"
  FRONT_IMAGE="${FRONT_IMAGE:-$IMAGE_REPOSITORY-front:$IMAGE_TAG}"
  MIGRATE_IMAGE="${MIGRATE_IMAGE:-$IMAGE_REPOSITORY-migrate:$IMAGE_TAG}"
