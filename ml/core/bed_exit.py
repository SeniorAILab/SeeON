...
class BedExitMonitor:
    ...
    Bed ids are deterministic: callers pass the cached bed boxes in detect_beds order
    and tuple index becomes bed_id. The monitor tracks person boxes with
    GreedyIouTracker, assigns a bed only after a stable containment hold period,
    and emits an exit only when a person leaves their own assigned bed after the
    grace window. Entering another bed suppresses cross-bed false positives.
    """
