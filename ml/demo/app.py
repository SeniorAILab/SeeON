...
(NestJS).  See the ADR for the demo/product surface boundary and live
...
# frame (train/serve parity, ADR; see demo-live-inference-frame-parity).
...
def _list_videos() -> list[videos.RegisteredVideo]:
    ...
    every internal data source is listed (ADR — demo is local-only).
    """
...
def _render_live_viewer(
    ...
    # windows match the training/eval pipelines (ADR anti-skew); only the
    # placeholder repaint is decimated via render_due — that incremental
    # render IS the live view (ADR). Pacing targets the clip's native fps
    ...
    try:
        for overlay, status, confidence in iter_live_frames(
            ...
            # the raw per-frame status below stays untouched (ADR — the
