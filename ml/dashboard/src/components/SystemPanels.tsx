...
export function StorageGauge({ system }: { system: SystemSnapshot | null }): JSX.Element {
  ...
  if (typeof usedPct !== 'number' || !Number.isFinite(usedPct)) {
    return (
      <div className="rounded-3xl bg-surface p-5 shadow-soft">
        <p className="text-sm font-black text-brand">스토리지 게이지</p>
        <h3 className="mt-1 text-xl font-black text-ink">클립 스토어 사용량</h3>
        <p className="mt-3 text-sm leading-6 text-ink-soft">
          현재 /api/v1/system 응답에 storage.clip_store.used_pct 값이 없습니다. 스토리지 사용률이 연결되면 이 영역에 표시됩니다.
        </p>
        <div className="mt-4 h-3 rounded-full bg-surface2">
  ...
  return (
    <div className="rounded-3xl bg-surface p-5 shadow-soft">
      <p className="text-sm font-black text-brand">스토리지 게이지</p>
      <h3 className="mt-1 text-xl font-black text-ink">클립 스토어 사용량</h3>
      <div className="mt-4 h-3 rounded-full bg-surface2" role="meter" aria-label="클립 스토어 사용률" aria-valuemin={0} aria-valuemax={100} aria-valuenow={percent}>
        <div className="h-3 rounded-full bg-brand" style={{ width: `${percent}%` }} />
      </div>
      <p className="mt-3 text-sm font-bold text-ink-soft">{usageText}</p>
    </div>
  );
}
      ...
      {entries && entries.length > 0 ? (
        <div className="mt-3 space-y-3">
          {entries.map((entry, index) => (
            <div key={entry.id ?? `${title}-${index}`} className="rounded-2xl bg-surface2 px-4 py-3">
              <p className="text-sm font-black text-ink">{entry.version ?? '버전 정보 없음'}</p>
        ...
        <p className="mt-3 text-sm text-ink-soft">제공된 이력 데이터가 없습니다. 배포 이력이 연결되면 이 영역에 표시됩니다.</p>
      )}
    </article>
  );
}
...
export function SystemPanel({
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-4xl bg-surface/85 p-6 shadow-soft">
        <p className="text-sm font-black tracking-[0.24em] text-brand">{apiBase}/system</p>
        <h2 className="mt-2 text-2xl font-black text-ink">시스템</h2>
        {systemError ? <p className="mt-3 text-sm font-bold text-rose-600">{systemError}</p> : null}
        {system ? (
          <p className="mt-3 text-sm text-ink-soft">API 버전 {system.version} · 마지막 백엔드 성공 {formatTime(system.backend.last_ok_at)}</p>
        ) : null}
      ...
      <section className="rounded-4xl bg-surface/80 p-5 shadow-soft">
        <p className="text-sm font-black text-brand">시스템 알림 센터</p>
        <h3 className="mt-1 text-xl font-black text-ink">/api/v1/status ops 이벤트</h3>
        <div className="mt-4 space-y-3">
          {events.length > 0 ? events.map((event) => (
            <article key={`ops-${event.id}`} className="rounded-3xl bg-surface2 p-4">
              <div className="flex items-start justify-between gap-3">
                <h4 className="font-black text-ink">{event.title}</h4>
                <time className="text-xs font-bold text-ink-faint">{formatTime(event.timestamp)}</time>
              </div>
              <p className="mt-2 text-sm text-ink-soft">{event.detail}</p>
            </article>
          )) : <p className="rounded-3xl bg-surface2 p-5 text-sm font-bold text-ink-soft">현재 렌더링할 ops 이벤트가 없습니다.</p>}
        </div>
      </section>
    </section>
  );
