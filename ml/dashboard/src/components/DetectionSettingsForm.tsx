...
import type { Camera, CameraPatchInput } from '../api/client';
...
export function DetectionSettingsForm({ cameras }: DetectionSettingsFormProps): JSX.Element {
  ...
  return (
    <form className="rounded-4xl bg-white/80 p-6 shadow-soft" onSubmit={handleSubmit} noValidate>
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.2em] text-indigo-500">Detection policy</p>
          <h2 className="mt-1 text-2xl font-black text-slate-950">탐지 설정</h2>
          <p className="mt-2 text-sm text-slate-500">카메라별 threshold, 도메인, 침대 수, 야간 시간창을 설계합니다.</p>
        </div>
        <button type="submit" className="rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white hover:bg-indigo-700">
          설정 저장
        </button>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <label className="block text-sm font-bold text-slate-700">
            야간 시작
            <input type="time" value={nightStart} onChange={(event) => setNightStart(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none ring-indigo-200 focus:ring-4" />
          </label>
          <label className="block text-sm font-bold text-slate-700">
            야간 종료
            <input type="time" value={nightEnd} onChange={(event) => setNightEnd(event.target.value)} className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none ring-indigo-200 focus:ring-4" />
          </label>
        </div>
      </div>

      <fieldset className="mt-5 rounded-3xl bg-slate-50 p-4">
        <legend className="px-2 text-sm font-black text-slate-700">도메인 on/off</legend>
