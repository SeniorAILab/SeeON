...
from worker.edge_worker import _config_refresh
...
def runtime_exit_events(
    ...
    *,
    ...
    return monitor.update(observation_for(box(60, 0, 160, 100), bed), time_sec=1.0)
...
def test_resolve_night_window_prefers_pulled_over_yaml() -> None:
    ...
    assert resolve_night_window(yaml_config, pulled) == NightWindow(
        start="22:00",
        end="06:00",
        tz="UTC",
    )
...
def test_resolve_night_window_falls_back_to_yaml() -> None:
    ...
    assert resolve_night_window(yaml_config, None) == NightWindow(
        start="21:00",
        end="05:00",
        tz="Asia/Seoul",
    )
...
def test_resolve_night_window_returns_none_without_pulled_or_yaml_window() -> None:
    assert resolve_night_window(_yaml_config(night_window=None), None) is None
