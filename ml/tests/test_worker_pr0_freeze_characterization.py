...
worker->ml-api relay paths in ``ml/worker/edge_worker_config.py`` and the shared
``_RunnerBundle`` runner identity are load-bearing and must remain green under
``ml/tests/test_worker_backend_ingest_contract.py`` and
``ml/tests/test_worker_runner_sharing.py``.
"""
...
from worker import camera_worker as cw
