...
def fetch_citations(ids: list[str]) -> dict[str, "int | None"]:
    ...
    results: dict[str, "int | None"] = {paper_id: None for paper_id in ids}
    ...
    for caller_id, s2_id in zip(ids, s2_ids):
        s2_to_caller[s2_id] = caller_id
    ...
    while batch_start < len(s2_ids):
        ...
        try:
            ...
            # On 429 Too Many Requests, back off and retry once
            if exc.code == 429:
        ...
        for s2_id, paper in zip(batch, data):
            caller_id = s2_to_caller.get(s2_id, s2_id)
            if paper is None:
                results[caller_id] = None
            else:
                results[caller_id] = paper.get("citationCount")
