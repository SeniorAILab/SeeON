...
4. Backend validates state, exchanges the code, stores/updates Kakao identity, sets the session cookie, then redirects:
   ...
   - `/dashboard/admin` for `ADMIN` users who already have a facility.
   - `/dashboard/staff` for `STAFF` users who already have a facility.
...
7. Super-admin facility selection uses `GET /api/v1/facilities` and `POST /api/v1/facilities/selection` with the backend-issued opaque `selectionToken`. Tenant resource routes then use the active facility stored in the server session; the frontend must not send `X-Facility-Id`, a `facilityId` query value, an internal `Facility.id`, or a customer facility code as tenant scope.
...
For facility-less `SUPER_ADMIN`, tenant resource routes require a previously selected active facility in server session state. Facility-bound `ADMIN`/`STAFF` accounts always use their session facility; client-provided facility headers, query values, route ids, and customer codes are ignored for access control.

### Facility

| Method | Path | Body | Response |
|---|---|---|---|
| GET | `/api/v1/facilities` | none | role-aware facility selector list: `SUPER_ADMIN` receives all facilities with opaque `selectionToken`; facility-bound users receive only their own facility |
| POST | `/api/v1/facilities/selection` | `{ selectionToken: string }` | stores active facility scope in server session and returns the selected facility |
