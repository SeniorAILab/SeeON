...
Recover product intent from the deleted 2F POC surface into the Ataraxia PRD without restoring legacy product routes or POC-only UI code.
...
Update the Ataraxia Obsidian PRD outside the repo to include:
...
- Direct file read of the Ataraxia PRD contains:
  - `현재 확인 필요 큐`
  - `추천 순찰 순서`
  - `Pilot UX 검증 모드`
