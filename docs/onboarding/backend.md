...
| Ports/adapters | 외부 시스템 호출과 failure translation을 소유한다. service는 concrete adapter가 아니라 port/token에 의존한다. | `backend/src/alerts/ports/channel.port.ts`, `backend/src/alerts/adapters/kakao-send-to-me-channel.adapter.ts`, `backend/src/alerts/ports/prediction.port.ts`, `backend/src/alerts/adapters/ml-serving-prediction.adapter.ts` |
