...
| [ADR-047](./backend/ADR-047-canonical-ingest-single-ingress-cleanup.md) | Canonical ingest single ingress cleanup | Accepted | 2026-06-18 | Supersedes ADR-043's pilot clause: `/ingest/alerts` is the only live alert ingress, and `/api.alerts/events` is removed from the live contract. |
