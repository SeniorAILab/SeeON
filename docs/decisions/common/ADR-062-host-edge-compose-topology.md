...
- Related: ADR-029 (edge inference deployment topology), ADR-048 (ml/backend window predict seam), ADR-023 (ML/backend prediction boundary), ADR-002 (Postgres everywhere), ADR-055 (Vite + React front stack), ADR-067 (ML edge API/worker split), ADR-071 (login/auth SSOT).
...
1. **ML runs on the edge.** Inference happens on the external edge device, which performs detection locally and pushes no-HMAC events to backend `POST /api/v1/events` (ADR-029). The backend's former `MlServingPredictionAdapter` pull seam (`ML_SERVING_URL` → `/predict`) was dormant on the live edge-push path and has since been removed (ADR-048/ADR-029, ref #431). Co-locating `ml-serving` with backend on the host contradicts the edge deployment model.
...
2. **Front is served by nginx with a same-origin reverse proxy.** A new multi-stage `front/Dockerfile` builds the Vite SPA and serves it from `nginx:1.27-alpine` on port 3000. `front/nginx.conf` serves the static SPA with a `try_files … /index.html` fallback and reverse-proxies the backend's two route prefixes — `/api/`, `/auth/` — to `http://backend:8080`, with `proxy_buffering off` scoped to `/api/sse`. The backend has no global `/api` prefix (see `docs/api/route-inventory.md`), so each prefix is proxied explicitly. Same-origin makes browser CORS a non-issue.
3. **ML moves to `compose.edge.yaml`.** The edge-only compose file builds explicit images from `ml/Dockerfile.api` and `ml/Dockerfile.worker` (ADR-068): `ml-api` for FastAPI health/status/relay gateway routes and backend Event API relay, and `ml-worker` for production RTSP camera ownership and live ML. The worker consumes a mounted `EDGE_CAMERA_CONFIG` YAML with per-camera RTSP URLs, local `ml-api` relay URL/token, and the LSTM fall-model artifact contract; backend `API_BACKEND_EVENTS_URL` configuration is held by `ml-api` per ADR-067/029. Because the edge is on a separate host/network, `ml-api` Event API URLs must use the backend's public endpoint — a Docker service name cannot reach it.
4. **`ML_SERVING_URL` is removed from the deployed backend env; the dormant code seam was removed later by ref #431.** Do not re-add backend-pull prediction without a successor ADR; live ML output is pushed through the Event API as `confidence`.
5. **DB stays co-located on the host + gets periodic backups.** The `db` service keeps its current shape (postgres:17-alpine, RLS roles `fall`/`fall_app`, `backend/prisma/init`, healthcheck, `pgdata` volume). `scripts/db-backup.sh` (pg_dump custom format + rotation) adds durability, with the restore procedure documented in the script header. Managed-Postgres migration is explicitly out of scope.
...
- Front defaults to real backend mode (`VITE_USE_MOCK` unset or `false`) so the nginx same-origin proxy is the normal local/prod path. Login/session/facility onboarding is backend-direct in dev/prod through email/password `POST /auth/login`, Kakao OAuth, `/auth/session`, and `POST /api/facilities`; explicit `VITE_USE_MOCK=true` remains available only for automated tests and is not part of dev/prod runtime (the front-alone mock "demo" path is retired; canonical doctrine in [../../architecture.md](../../architecture.md)), while remaining dashboard/admin service wiring is replaced incrementally.

## Follow-ups

- Replace remaining mock-backed dashboard/admin frontend service workflows with backend endpoint calls.
- Real production deploy execution, Event API transport hardening and CI for the compose lanes.

## Changelog

- 2026-06-25: Repointed the DB-durability reference from the removed `docs/runbooks/db-backup-restore.md` to `scripts/db-backup.sh`, whose header now documents the backup and clean-volume restore procedure. Decision unchanged.

- 2026-06-27: Aligned the front mock wording with the corrected demo/mock doctrine — `VITE_USE_MOCK=true` is automated-tests-only and not part of dev/prod runtime; the front-alone mock "demo" path is retired. Canonical doctrine: `docs/architecture.md`. Decision unchanged.
