...
3. **ML moves to `compose.edge.yaml`.** The edge-only compose file builds `ml/Dockerfile` as two services: `ml-edge-api` for FastAPI health/status/debug routes and `ml-edge-worker` for production RTSP camera ownership. The worker consumes a mounted `EDGE_CAMERA_CONFIG` JSON with per-camera RTSP URLs and ingest credentials. Because the edge is on a separate host/network, ingest URLs in that file must use the backend's public endpoint — a Docker service name cannot reach it.
