...
```text
Browser -> http://101.79.18.95/login
Kakao login button -> /auth/kakao/login
Backend -> https://kauth.kakao.com/oauth/authorize
Kakao redirect -> http://101.79.18.95/auth/kakao/callback
Backend callback -> app_session cookie -> http://101.79.18.95/dashboard or /onboarding
...
3. Register the exact REST API redirect URI:
   ...
   - current public site: `http://101.79.18.95/auth/kakao/callback`
...
11. If Kakao messages contain links to `http://101.79.18.95`, register the website domain/product link domain required by the Kakao message product.
...
1. `http://101.79.18.95/auth/kakao/login` redirects to Kakao with `redirect_uri=http://101.79.18.95/auth/kakao/callback`.
...
4. `GET http://101.79.18.95/auth/session` returns `{ user }` with cookies.
