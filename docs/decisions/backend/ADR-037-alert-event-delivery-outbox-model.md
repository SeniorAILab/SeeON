# ADR-037: Postgres alert event and delivery outbox model
...
`AlertEvent`/`DeliveryAttempt` are backend-owned, `source_id`-keyed outbox tables — not org-scoped tenant data — so they are intentionally excluded from the `PrismaService.TENANT_MODELS` RLS guard (ADR-032) and require no org context.
