...
Before facility binding, a locally authenticated user may have no tenant scope. After onboarding, backend sessions carry the tenant scope used by guards and tenant-bound database access. Kakao OAuth local-account provisioning is governed by ADR-071 and is not decided here.
...
Rename onboarding from `POST /api/orgs` to `POST /api/facilities`. The request body supplies the facility name only; it does not supply `facilityId`.
...
- Keep Kakao login and local-account provisioning behavior owned by ADR-071 unchanged.
