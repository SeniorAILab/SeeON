# ADR-034: SSE Realtime Transport — Read-Only Cookie-Auth Push with alertSeq Replay
...
`alertSeq` is the `Alert.alertSeq` bigserial (ADR-031) — a monotonically increasing integer, not
...
current-state grid (resident status badges) is derived from `ResidentStatus` (ADR-031), not the
...
- `alertSeq` is a `BigInt` in Postgres/Prisma; must be serialized as a string in JSON payloads
  and the SSE `id` field (ADR-031 follow-up).
- The interleaved-insert reconnect test (concurrent inserts + drop/reconnect) is a required
  acceptance test for AC8, not optional. It must run as the `fall_app` DB role (ADR-032).
