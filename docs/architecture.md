...
| [ADR-022 — B2B facility multitenancy (Postgres RLS + NOSUPERUSER role + composite FK)](decisions/ADR-022-b2b-facility-multitenancy-rls.md) | Tenant isolation is a DB-level default-deny invariant: RLS ENABLE+FORCE on every tenant table, `current_setting('app.current_org_id')` policy, per-request `SET LOCAL` GUC inside a transaction, dedicated `NOSUPERUSER NOBYPASSRLS` app runtime role, composite FKs making cross-org child rows unrepresentable |
| [ADR-023 — Kakao OAuth auth boundary (backend-owned callback, httpOnly session JWT)](decisions/ADR-023-kakao-oauth-auth-boundary.md) | Backend owns OAuth callback + session minting; single httpOnly `app_session` JWT cookie with `ServerSession` for revocation + sliding rotation; single browser-facing origin via Next.js rewrites so cookie is first-party for fetch and EventSource; no NextAuth |
...
| [ADR-025 — SSE realtime transport](decisions/ADR-025-sse-realtime-transport.md) | `GET /sse` (Next rewrite) pushes cookie-authenticated events with `id: alertSeq`; `Last-Event-ID` replay + REST `GET /status` re-snapshot on reconnect; in-process write queue serializes alertSeq assign→commit→emit; multi-instance LISTEN/NOTIFY deferred; WebSocket rejected (bidirectional unneeded) |
