...
Service files such as `authService.ts` may keep mock-mode orchestration and domain workflows, but they should not directly call backend `fetch()` or cast backend JSON. The shared `apiClient` returns `unknown` JSON; endpoint mappers must parse it into typed frontend state.
