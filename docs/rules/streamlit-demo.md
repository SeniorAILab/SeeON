...
- **Inference is never strided.** The model consumes every consecutive frame
  ...
  pipelines (decision map anti-skew). Only the *repaint* is decimated — every
...
- **Latched event badge** — `FallEventLatch` (demo.live_view) turns rising
  ...
  it never invents or extends a fall state (decision map); the raw per-frame
  status stays untouched. Product alerting (ack, notifications) is backend
  scope (decision map), not the demo's.

## 3. Independent overlay toggles

Bounding boxes and the pose skeleton are **independent render options** on a
single `DetectionResult` (per decision map), not separate models. Expose each as
...
(decision map, superseding
decision map; non-product per decision map).
...
- **Never expose this demo externally as-is** — it lists patient-adjacent
  ...
  data-access-boundary decision first (decision map). Footage custody (footage stays on
  operator disks, out of Git) is owned by
  decision map, not a demo runtime mode.
...
and downstream consumers untouched (decision map). Do not branch the UI on
framework internals.
...
- **First-select latency.** `yolo26{s,m,l,x}-pose.pt` weights download on first
  ...
  and decision map. `*.pt` is gitignored — never commit weights. Expect a one-time
...
- **Never fabricate data.** Nothing in the demo may paint keypoints, boxes, or
  labels that did not come from a real model inference (decision map).
...
**Allowed (not a bypass):** pose extraction and temporal classification stay **edge-local** in the demo harness. That mirrors the edge-local half of decision map, while the production service that performs live classification is `ml-worker`, not `ml-api`.
