...
스크린샷은 실제 TV 해상도(1920×1080)로 찍었다. 7개 방이 한 층에 있을 때
**타일이 화면 위쪽 약 30%만 쓰고 아래 70%가 빈다.** 하니스 문제가 아니라
`FloorMonitorPage:190`의 실제 구조(`flex min-h-0 flex-1`)에서 나온 결과다 —
층 그룹이 내용 높이만큼만 차지한다.

4m 거리에서는 타일이 클수록 좋으므로 남는 공간을 쓰는 편이 낫다. 다만
레이아웃 변경은 디자인 결정이라 야간에 임의로 바꾸지 않았다.

- **오늘은 이대로 가도 된다**: 글자 크기 자체는 4m 기준을 통과한다
  (`-t readability` 3 passed). 빈 공간이 가독성을 해치지는 않는다.
- **바꾸려면**: 층 그룹이 남은 높이를 채우도록 하는 변경이 필요하다.
  별도 승인 후 진행한다.
