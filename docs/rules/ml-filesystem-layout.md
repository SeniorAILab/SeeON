...
> [ADR-011](../decisions/ADR-011-ml-data-domain-first-layout.md) (and
...
| Domain-bound data, any role | `ml/data/{domain}/{raw,processed,poses,annotated}` | no (gitignored) | ADR-011 |
| Cross-domain derived outputs | `ml/data/eval/` | no (gitignored) | ADR-011 |
| Transient demo uploads | `ml/data/uploads/` | no (gitignored) | ADR-011 |
...
- **Raw is sacred.** Files under any `{domain}/raw/` are never modified in
  ...
  (inherited from ADR-004 via ADR-011).
- **`ml/data/` is partitioned domain-first.** Data goes in
  ...
  — that drift is exactly what ADR-011 removed.
- **`nursing-home/` is operator-only.** It must never be listed or served by
  ...
  input surface (ADR-011 Access Boundary; mechanism in
...
See [ADR-011](../decisions/ADR-011-ml-data-domain-first-layout.md) for the full
