...
| [worktree-workflow.md](./worktree-workflow.md) | repo-wide | Branch&worktree facet (hub: version-control.md). Never work directly on a protected branch; every issue maps to a branch `<type>/<issue#>-<slug>` and one worktree; use `git wt <issue#>` to create; per-task vs. lane-pool lifecycle. Records ADR-008. |
